...
contract ModelAnime{
    ...
    function incrementView(string calldata _id) external returns (bool){
        views[_id] += 1;
        return true;
    }
    
    function incrementStar(string calldata _id) external returns (bool){
        star[_id] += 1;
        return true;
    }
    
    function incrementComment(string calldata _id) external returns (bool){
        comment[_id] += 1;
        return true;
    }
    
    function transferModel(address _fromAddr, address _toAddr, string calldata _id, uint _nonce) external returns (bool){
    ...
    function createModel(address _fromAddr,string calldata _id, string calldata _name, string calldata _madeFrom, bool _limited, uint price) external onlyAdmin returns (bool){
    ...
    function showModelById(address _ownerAddr,string calldata _id) external view returns(Model memory){
        require(_ownerAddr != address(0), "invalid address");
    ...
    function showView(string calldata _id) external view returns(uint){
        return views[_id];
    }
    
    function showStar(string calldata _id) external view returns(uint){
        return star[_id];
    }
    
    function showComment(string calldata _id) external view returns(uint){
        return comment[_id];
    }
    ...
    function showOwner(address _ownerAddr, string calldata _id) external view returns(bool){
